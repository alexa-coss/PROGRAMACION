...
def rotacion(ruta_video):            
    ...
    out_w, out_h = (height, width) if rotacion in [90, 270] else (width, height)

    fourcc = cv2.VideoWriter_fourcc(*'mp4v') # Definir códec (formato) para guardar video
    out = cv2.VideoWriter(ruta_salida_temp, fourcc, fps, (out_w, out_h)) # Preparar objeto VideoWriter para guardar nuevo video corregido

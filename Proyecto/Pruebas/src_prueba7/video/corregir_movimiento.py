...
from video.roi import reset_roi_state       
...
def _auto_roi_mediapipe(cap, max_busqueda=90, prefer="auto"):
    ...
    with mp_face.FaceMesh(
            ...
            min_detection_confidence=0.35, # Umbral mínimo de confianza para detección inicial
            min_tracking_confidence=0.35   # Umbral mínimo de confianza para seguimiento posterior
...
def _redetectar_ojo_en_frame(frame_bgr, lado_preferido):
    """Re-detecta el ROI del iris del lado indicado en un único frame."""
    H, W = frame_bgr.shape[:2]
    if _USA_MEDIAPIPE:
        with mp_face.FaceMesh(static_image_mode=False, refine_landmarks=True,
                              max_num_faces=1, min_detection_confidence=0.35,
                              min_tracking_confidence=0.35) as face_mesh:
            rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
            res = face_mesh.process(rgb)
            if not res.multi_face_landmarks:
                return None
            lm = res.multi_face_landmarks[0].landmark
            def puntos(idx): return [(lm[i].x*W, lm[i].y*H) for i in idx]

            iris = puntos(LEFT_IRIS_IDX if lado_preferido == "left" else RIGHT_IRIS_IDX)
            if len(iris) != 4:
                return None
            cx, cy, r = _center_radius(iris)
            m = 2.5
            x1 = int(cx - m*r); y1 = int(cy - m*r)
            w  = int(2*m*r);   h  = int(2*m*r)    
            x1, y1, w, h = _clip_bbox(x1, y1, w, h, W, H)
            return (x1, y1, w, h)
    else:
        return None
...
def corregir_movimiento(ruta_video, salida=None, prefer_side="auto", debug=True):
    ...
    side_fijo = side                                             # Guardar lado elegido (left/right)
    ...
    while True:  # Procesar cada frame de video hasta que termine o se interrumpa
        ...
        if not paused:
            ...
            if score < THRESH_BAD:                                              # Si el matching es malo
                redet_bbox = _redetectar_ojo_en_frame(frame, side_fijo)
                if redet_bbox is not None:
                    x0, y0, w0, h0 = redet_bbox             # Actualizar ROI de referencia
                    templ = gray[y0:y0+h0, x0:x0+w0].copy() # Re-armar plantilla
                    ref_tl = (x0, y0)                       # Re-fijar referencia
                    last_tl = ref_tl
                    cx, cy = ref_tl                         # Evitar salto de ojo
                else:
                    cx, cy = last_tl                        # Mantener última válida
            else:
                cx, cy = tl
                last_tl = tl
            ...
            cv2.putText(vis, f"score:{score:.2f}  side:{side_fijo}  FPS:{fps_rt:.1f}", (20, 30), # Mostrar score, lado y FPS
    ...
    return salida, side_fijo                   # Devolver ruta video de salida

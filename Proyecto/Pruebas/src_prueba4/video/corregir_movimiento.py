...
def corregir_movimiento(ruta_video):
    ...
    roi = fijarPunto(prev_frame)  # Devuelva rectángulo completo
    ...
    while True: # Leer cada frame
        ...
        cv2.imshow("Video corregido", frame_corregido)  # Mostrar frame corregido en ventana
...
def fijarPunto(frame):
    ...
    roi = cv2.selectROI("Seleccionar recuadro fijo", frame, fromCenter=True, showCrosshair=True)

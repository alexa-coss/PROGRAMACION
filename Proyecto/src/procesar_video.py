...
def procesar_video(ruta_video_original):
    ...
    ruta_video_corregido, lado_fijo = corregir_movimiento(ruta_video_rotado, prefer_side="auto", ojos="both", debug=True)  # Usar estabilización con ambos ojos cuando sea posible

    # Validar salida de estabilización
    if not ruta_video_corregido:  # Validar que exista ruta de salida
        print("No se pudo generar el video estabilizado")
        return  # Delegar manejo de error al llamador

    # Paso 4: seguimiento con video corregido
    seguimiento(ruta_video_corregido, modo_ojos="both", lado_fijo=lado_fijo)  # Usar seguimiento de ambos ojos
